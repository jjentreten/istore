...
// ── Gateway selector ──────────────────────────────────────────────────────────
// Alterne entre "brutal" e "blackcat" via variável PAYMENT_GATEWAY no .env
...
function buildUtmifyPayload({ orderId, status, createdAt, approvedDate, customer, products, tracking, totalPriceInCents }) {
  ...
  return {
    ...
    customer: {
      name: customer.name,
      email: customer.email || "cliente@istore.com.br",
...
app.post("/api/criar-pix", async (req, res) => {
  const { customer, items = [], tracking, shipping } = req.body;
  ...
  if (GATEWAY === "brutal") {
    // ── BrutalCash ────────────────────────────────────────────────────────────
    const orderId = `ct_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
    let transaction;
    try {
  ...
  // Gera QR Code local se a resposta não trouxer imagem
  if (pixCode) {
    try {
...
app.get("/api/status-pix", async (req, res) => {
  ...
  // Detecta o gateway pelo registro pendente (mantém compatibilidade ao alternar)
  const pending = readPending();
...
app.post("/api/cupom-validar", (req, res) => {
  ...
  const cupons = {
    ISTORE10: { desconto: 0.10, tipo: "percentual" },
  };
  const cupom = cupons[(codigo || "").toUpperCase()];
  if (!cupom) return res.json({ valido: false, motivo: "Cupom inválido ou expirado." });
...
app.listen(PORT, () => {
  console.log(`iStore rodando na porta ${PORT}`);
